...
function goPage(p){
  ...
  if(p==='mushaf'){
    ...
    document.getElementById('backBtn').style.display='none';document.getElementById('hdrTitle').textContent='المصحف';document.getElementById('hdrSub').textContent='مصحف المدينة النبوية';renderV9Mushaf();
...
function bulkSetStatus(status){if(!V9.selectedStudents.size)return v9Toast('حدد طالبًا واحدًا على الأقل','error');const ids=[...V9.selectedStudents],before=new Map(ids.map(id=>{const s=students.find(x=>x.id===id);return[id,s?v9StudentStatus(s):'active'];}));students.forEach(s=>{if(V9.selectedStudents.has(s.id)){s.studentStatus=status;s.updatedAt=new Date().toISOString();}});save();V9.undo=()=>{before.forEach((old,id)=>{const s=students.find(x=>x.id===id);if(s)s.studentStatus=old;});save();renderSt();};setTimeout(()=>V9.undo=null,8000);V9.selectedStudents.clear();renderSt();updateBulkBar();v9Toast('تم تحديث حالة الطلاب — Ctrl+Z للتراجع','success');}
...
function openV9ContextMenu(e,id){e.preventDefault();V9.contextStudentId=id;const m=document.getElementById('v9ContextMenu'),st=students.find(s=>s.id===id);if(!m||!st)return;m.innerHTML=`<button onclick="startFor('${v9Esc(id)}');closeV9ContextMenu()">${v9Icon('play')} بدء حصة</button><button onclick="openProf('${v9Esc(id)}');closeV9ContextMenu()">${v9Icon('users')} ملف الطالب</button><button onclick="directWA('${v9Esc(id)}');closeV9ContextMenu()">${v9Icon('message')} واتساب</button><button onclick="toggleStudentPin('${v9Esc(id)}');closeV9ContextMenu()">${v9Icon('star')} ${st.pinned?'إلغاء التثبيت':'تثبيت'}</button><button onclick="setStudentStatusWithUndo('${v9Esc(id)}','archived');closeV9ContextMenu()">${v9Icon('trash')} أرشفة</button>`;m.hidden=false;m.style.left=Math.min(e.clientX,innerWidth-210)+'px';m.style.top=Math.min(e.clientY,innerHeight-250)+'px';}
function closeV9ContextMenu(){const m=document.getElementById('v9ContextMenu');if(m)m.hidden=true;}
function setStudentStatusWithUndo(id,status){const st=students.find(s=>s.id===id);if(!st)return;const old=v9StudentStatus(st);st.studentStatus=status;save();renderSt();V9.undo=()=>{st.studentStatus=old;save();renderSt();};v9Toast(`تم ${status==='archived'?'أرشفة':'تحديث'} ${st.name} — يمكنك التراجع من Ctrl+Z`,'info');setTimeout(()=>V9.undo=null,8000);}
...
function openProf(id){V9_BASE.openProf(id);injectProfileTabs();const b=document.querySelector('#v9ProfileTabs button');setProfileTab('plan',b);}
...
function enhanceQuranModal(){
  ...
  const audio=document.getElementById('quranAudioPlayer');if(audio){audio.removeEventListener('ended',V9_BASE.playNextQuranAudio);audio.addEventListener('ended',v9QuranEnded);audio.addEventListener('play',renderMiniPlayer);audio.addEventListener('pause',renderMiniPlayer);audio.addEventListener('timeupdate',renderMiniPlayer);}
}
