...
function sanitizeBackupData(d){
  ...
  safe.sessions.forEach((se,i)=>{const raw=(d.sessions||[])[i]||{};se.recitationErrors=(raw.recitationErrors||[]).filter(x=>x&&V8_ERROR_TYPES.includes(x.type)).slice(0,1000).map(x=>({type:x.type,at:v8SafeText(x.at,40)}));if(/^[A-Za-z0-9._:-]{1,160}$/.test(String(raw.voiceNoteId||'')))se.voiceNoteId=raw.voiceNoteId;if(Array.isArray(raw.reviewAssignments))se.reviewAssignments=JSON.parse(JSON.stringify(raw.reviewAssignments));if(Array.isArray(raw.reviewResults))se.reviewResults=JSON.parse(JSON.stringify(raw.reviewResults));});return safe;
}
