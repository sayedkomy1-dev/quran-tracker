...
async function setupPinLock(){if(!crypto?.subtle)return v8Toast('التشفير غير مدعوم في هذا المتصفح','error');const a=prompt('أدخل PIN من 4 إلى 8 أرقام:');if(a==null)return;if(!/^\d{4,8}$/.test(a))return v8Toast('PIN يجب أن يكون من 4 إلى 8 أرقام','error');const b=prompt('أعد إدخال PIN للتأكيد:');if(a!==b)return v8Toast('PIN غير متطابق','error');settings.security={...(settings.security||{}),lockEnabled:true,pinHash:await hashText(a)};save();renderSecurityStatus();v8Toast('تم تفعيل قفل التطبيق','success');}
async function disablePinLock(){if(!settings.security?.lockEnabled)return;const p=prompt('أدخل PIN الحالي لإلغاء القفل:');if(p==null)return;if(settings.security.pinHash&&await hashText(p)!==settings.security.pinHash)return v8Toast('PIN غير صحيح','error');settings.security={lockEnabled:false,pinHash:'',credentialId:''};save();hideAppLock();renderSecurityStatus();v8Toast('تم إلغاء قفل التطبيق','success');}
...
async function unlockWithPin(){const p=document.getElementById('lockPinInput')?.value||'',e=document.getElementById('lockError');if(!settings.security?.pinHash){if(settings.security?.credentialId)return unlockWithBiometric();if(e)e.textContent='لا يوجد PIN مضبوط';return;}if(await hashText(p)===settings.security.pinHash)hideAppLock();else{if(e)e.textContent='PIN غير صحيح';document.getElementById('lockPinInput')?.select();}}
